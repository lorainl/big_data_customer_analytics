...
dev.off()
hotel_bookings$meal_plan_yn<-ifelse(hotel_bookings$type_of_meal_plan==0,0,1)
prop.table(table(hotel_bookings$booking_status, hotel_bookings$meal_plan_yn>0.5))
...
hotel_bookings$glm_fwd_pred<-predict(glm_model.forward,hotel_bookings, type="response")
prop.table(table(hotel_bookings$booking_status, hotel_bookings$glm_fwd_pred>0.5))
...
model <- keras_model_sequential() %>%
    layer_flatten() %>%
    layer_dense(units = 6, activation = "relu",input_shape = c(18)) %>%
